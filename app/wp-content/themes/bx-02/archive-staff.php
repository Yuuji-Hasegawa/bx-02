...
<div class="o-container o-container:underPage u-pt-l u-pb-l">
    <?php if (have_posts()):?>
    <ul class="o-switcher o-switcher:staff">
        <?php while (have_posts()): the_post();?>
        <li class="o-stack o-stack:staff-item">
            ...
            <dl>
                <dt class="c-staff-name">
                    <?php get_post_meta($post->ID, 'staff_job', true) ? $job = get_post_meta($post->ID, 'staff_job', true) : $job = 'スタッフ'; echo $job;?>
                    <span class="c-staff-name__font-large"><?php the_title();?></span>
                </dt>
                <dd class="o-stack o-stack:xs">
                    <?php
                        $body = get_the_content($post->ID);
                        if ($body) {
                            echo '<b>経歴</b><span>' . $body . '</span>';
                        }
                        $message = get_post_meta($post->ID, 'staff_message', true);
                        if ($message) {
                            echo '<b>メッセージ</b><span class="u-mb-m">' . $message . '</span>';
                        }
                        ...
                        if ($sns) {
                            echo '<div class="o-cluster o-cluster:staffSns">';
                            foreach ($sns as $elem) {
                                echo '<a class="c-link c-link:staffSns" href="' . $elem['url'] . '" target="_blank" rel="noopener">' . $elem['icon'] . '</a>';
                            }
                            echo '</div>';
                        }
                    ?>
